import Head from "next/head";
import Image from "next/image";
import styles from "../styles/Home.module.css";
import Script from "next/script";

export default function Home() {
  return (
    <div className={styles.page}>
      <Head>
        <title>Landing-1</title>
        <meta name="description" content="Generated by Nevyl" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Script
        src="https://www.googletagmanager.com/gtag/js?id=GTM-TWRN7MB"
        strategy="afterInteractive"
      />
      <Script id="google-analytics" strategy="afterInteractive">
        {`
           window.dataLayer = window.dataLayer || [];
           function gtag(){dataLayer.push(arguments);}
           gtag('js', new Date());
         
           gtag('config', 'GTM-TWRN7MB');
        `}
      </Script>
      <div className={styles.menu}>
        <div className={styles.menu_logo}>
          <Image src="/img/img1.png" alt="Logo" width={352} height={177} />
        </div>
        <div className={styles.btn}>
          <a
            href="https://apps.apple.com/us/app/dr-mooves-a-d/id1528375544"
            rel="nofollow"
          >
            <span>
              DOWNLOAD APP
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                viewBox="0 0 16 16"
              >
                <path
                  fillRule="evenodd"
                  d="M3.5 10a.5.5 0 0 1-.5-.5v-8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 .5.5v8a.5.5 0 0 1-.5.5h-2a.5.5 0 0 0 0 1h2A1.5 1.5 0 0 0 14 9.5v-8A1.5 1.5 0 0 0 12.5 0h-9A1.5 1.5 0 0 0 2 1.5v8A1.5 1.5 0 0 0 3.5 11h2a.5.5 0 0 0 0-1h-2z"
                />
                <path
                  fillRule="evenodd"
                  d="M7.646 15.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 14.293V5.5a.5.5 0 0 0-1 0v8.793l-2.146-2.147a.5.5 0 0 0-.708.708l3 3z"
                />
              </svg>
            </span>
          </a>
        </div>
      </div>
      <main className={styles.main}>
        <div className={styles.container}>
          <header className={styles.section_flex}>
            <div className={styles.section_1}>
              <div className={styles.title}>
                Get paid to treat your child&apos;s anxiety and depression
                symptoms!
              </div>
              <div className={styles.subtitle}>
                Get your child’s symptoms virtually treated by Dr. MOOVES app
                and <b>get paid!</b>
              </div>
              <div className={styles.subtitle}>
                Receive checks from your insurance company by treating your
                children’s A&D symptoms.
              </div>
              <div className={styles.title}>How does it work?</div>
              <div className={styles.subtitle}>
                <b> Be compensated for your child’s health!</b>
                <ul>
                  <li>
                    Dr. Mooves app is a fun dance virtual therapy, which records
                    their stability, sway, basal support, symmetry, synchrony
                    and much more.
                  </li>
                  <li>
                    After performing every dance, children get a different
                    prescription dance and a change in their improvement
                    potential.
                  </li>
                  <li>
                    The more progress your child shows, the better compensation
                    you will receive from your insurance company.
                  </li>
                  <li>
                    All dances grow their mental strength and cognitive
                    resiliency helping with anxiety and depression symptoms
                  </li>
                </ul>
              </div>
              <div className={styles.title}></div>
            </div>
            <div className={styles.section_2}>
              <div className={styles.video}>
                <div className={styles.logo}>
                  <Image
                    src="/img/img1.png"
                    alt="Logo"
                    width={352}
                    height={177}
                  />
                </div>
                <div>
                  <video width="100%" autoPlay muted loop>
                    <source src="/img/video.mp4" type="video/mp4" />
                    Your browser does not support HTML video.
                  </video>
                </div>
                <div className={styles.btn}>
                  <a
                    href="https://apps.apple.com/us/app/dr-mooves-a-d/id1528375544"
                    rel="nofollow"
                  >
                    <span>
                      DOWNLOAD APP
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        fill="currentColor"
                        viewBox="0 0 16 16"
                      >
                        <path
                          fillRule="evenodd"
                          d="M3.5 10a.5.5 0 0 1-.5-.5v-8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 .5.5v8a.5.5 0 0 1-.5.5h-2a.5.5 0 0 0 0 1h2A1.5 1.5 0 0 0 14 9.5v-8A1.5 1.5 0 0 0 12.5 0h-9A1.5 1.5 0 0 0 2 1.5v8A1.5 1.5 0 0 0 3.5 11h2a.5.5 0 0 0 0-1h-2z"
                        />
                        <path
                          fillRule="evenodd"
                          d="M7.646 15.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 14.293V5.5a.5.5 0 0 0-1 0v8.793l-2.146-2.147a.5.5 0 0 0-.708.708l3 3z"
                        />
                      </svg>
                    </span>
                  </a>
                </div>
              </div>
            </div>
          </header>
        </div>
      </main>
      <footer className={styles.footer}>
        Playing Forward Copyright 2022. All Rights Reserved.
      </footer>
    </div>
  );
}
